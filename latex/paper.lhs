...
\author{\IEEEauthorblockN{Samuel S. Feitosa}
\IEEEauthorblockA{\textit{Departamento de Informática} \\
\textit{Instituto Federal de Santa Catarina}\\
Caçador, SC - Brazil \\
samuel.feitosa [at] ifsc.edu.br}
\and
\IEEEauthorblockN{Rodrigo G. Ribeiro}
\IEEEauthorblockA{\textit{Prog. Pós Grad. em Ciência da Computação} \\
\textit{Universidade Federal de Ouro Preto}\\
Ouro Preto, MG - Brazil \\
rodrigo.ribeiro [at] ufop.edu.br}
\and
\IEEEauthorblockN{Andre R. Du Bois}
\IEEEauthorblockA{\textit{Prog. de Pós Grad. em Computação} \\
...
\begin{itemize}
\item We specify the static and dynamic semantics of FJ (class table and expressions) in Agda using the syntactic approach~\cite{Wright:1994:SAT:191905.191909}.
...
Section \ref{sec:mec} shows how we represent types, how we model the class table and expressions, and the specification of the static and dynamic semantics of FJ in Agda. Section \ref{sec:proofs} discusses the proof steps to guarantee type safety of the studied calculus. Section \ref{sec:eval} presents the steps to define evaluation through repeated applications of the \emph{progress} and \emph{preservation} theorems. Section \ref{sec:rel} discusses related work. Finally, we present the final remarks in Section~\ref{sec:conclusion}.
...
\begin{spec}
data fields : Name → List (Name × Name)
  → Set where
  obj    : fields Obj []
  other  : ∀ {c cd sf}
         → Δ ∋ c ∶ cd
...
\begin{spec}
{-"\hspace{-15pt}"-} data method : Name → Name → Meth → Set where
  this : ∀ {c cd m mdecl}
       → Δ ∋ c ∶ cd
...
\begin{spec}
data interl : List (Name × A) → List B
            → List (Name × B) → Set
...
\begin{spec}
{-"\hspace{-15pt}"-} data _⊢_∶_  : Ctx → Expr → Name → Set
{-"\hspace{-15pt}"-} data _⊧_∶_  : Ctx → List Expr → List Name → Set  
\end{spec}
...
\begin{spec}
{-"\hspace{-20pt}"-} data ClassOk : Class → Set where
{-"\hspace{-5pt}"-} T-Class : ∀ {CD}
{-"\hspace{-15pt}"-}         → All (MethodOk CD) (proj₂ (unzip (Class.meths CD)))
{-"\hspace{-15pt}"-}         → ClassOk CD
\end{spec}
...
\begin{spec}
{-"\hspace{-20pt}"-} data MethodOk : Class → Method → Set where
{-"\hspace{-5pt}"-} T-Method : ∀ {CD MD}
{-"\hspace{-20pt}"-}          → Meth.params MD ⊢ Meth.body MD ∶ Meth.ret MD
{-"\hspace{-20pt}"-}          → MethodOk CD MD
\end{spec}
...
\begin{spec}
{-"\hspace{-15pt}"-} preservation  : ∀ {e e' τ} → [] ⊢ e ∶ τ → e ⟶ e'
                                   → [] ⊢ e' ∶ τ
{-"\hspace{-15pt}"-} preservation (T-Var x) ()
{-"\hspace{-15pt}"-} preservation  (T-Field tp fls bnd) (RC-Field ev) =
                                   T-Field (preservation tp ev) fls bnd
{-"\hspace{-15pt}"-} preservation  (T-Field (T-New fs₁ tps) fs₂ bnd)
                                   (R-Field fs₃ zp bnde)  rewrite ≡-fields fs₁ fs₂ |
                                   ≡-fields fs₂ fs₃ = ⊢-interl zp tps bnd bnde
{-"\hspace{-15pt}"-} preservation  (T-Invk tp tmt tpl) (RC-InvkRecv ev) =
                                   T-Invk (preservation tp ev) tmt tpl
{-"\hspace{-15pt}"-} preservation  (T-Invk tp tmt tpl) (RC-InvkArg evl) =
                                   T-Invk tp tmt (preservation-list tpl evl)
{-"\hspace{-15pt}"-} preservation  (T-Invk (T-New fls cp) tmt tpl) (R-Invk rmt zp)
                                   rewrite ≡-method rmt tmt =
                                   subst (⊢-method tmt) tpl zp
{-"\hspace{-15pt}"-} preservation  (T-New fls tpl) (RC-NewArg evl) =
...
\begin{spec}
{-"\hspace{-15pt}"-} progress : ∀ {e τ} → [] ⊢ e ∶ τ → Progress e
{-"\hspace{-15pt}"-} progress (T-Var ())
{-"\hspace{-15pt}"-} progress (T-Field tp fls bnd) with progress tp
{-"\hspace{-15pt}"-} progress (T-Field tp fls bnd) | Step ev = Step (RC-Field ev)
{-"\hspace{-15pt}"-} progress  (T-Field (T-New flds fts) fls bnd) | Done ev
                               rewrite ≡-fields flds fls =
                                 Step (R-Field fls (proj₂ (⊧-interl fts))
                                 (proj₂ (∋-interl fts (proj₂ (⊧-interl fts)) bnd)))p
{-"\hspace{-15pt}"-} progress (T-Invk tp mt tpl) with progress tp
{-"\hspace{-15pt}"-} progress  (T-Invk tp mt tpl) | Step ev =
                               Step (RC-InvkRecv ev)
{-"\hspace{-15pt}"-} progress (T-Invk tp mt tpl) | Done ev with progress-list tpl
{-"\hspace{-15pt}"-} progress  (T-Invk tp mt tpl) | Done ev | Step evl =
                               Step (RC-InvkArg evl) 
{-"\hspace{-15pt}"-} progress  (T-Invk (T-New flds fts) mt tpl) | Done ev |
                               Done evl = Step (R-Invk mt (proj₂ (⊧-interl tpl)))
{-"\hspace{-15pt}"-} progress (T-New fls tpl) with progress-list tpl
{-"\hspace{-15pt}"-} progress (T-New fls tpl) | Step evl = Step (RC-NewArg evl)
{-"\hspace{-15pt}"-} progress (T-New fls tpl) | Done evl = Done (V-New evl)
\end{spec}
...
\begin{spec}
{-"\hspace{-15pt}"-} data _↠_ : Expr → Expr → Set where
{-"\hspace{-15pt}"-}   refl  : ∀ {e} → e ↠ e
{-"\hspace{-15pt}"-}   multi : ∀ {e e' e''} → e ⟶ e' → e' ↠ e'' → e ↠ e''
\end{spec}
